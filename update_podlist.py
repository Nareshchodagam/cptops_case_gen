...
# removed if condition as we are not using the else part (non_prod_dc)
    ...
    if re.search(r'crz', rolename, re.IGNORECASE):
        ...
        prod_dc = 'phx'
    elif re.search(r'public|^polcore|^pkicontroller|^grok|hbase|sam|dvasyslog|nwexp|dvamon|dvaexp|searchidx|searchmgr|dva_onboarding|^deepsea', rolename, re.IGNORECASE):
        prod_dc.extend(['prd'])
        ...
        prod_dc.extend(['crd'])
    elif re.search(r'praccn|praapp|pravmh|sdproxy|^gdsdclient|^servicedirectory', rolename, re.IGNORECASE):
        prod_dc.extend(['prd'])
    return prod_dc
    #else:
        #return non_prod_dc

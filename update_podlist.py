...
# removed if condition as we are not using the else part (non_prod_dc)
    ...
    if re.search(r'crz', rolename, re.IGNORECASE):
        ...
        prod_dc = 'was'
    elif re.search(r'public|^polcore|^pkicontroller|^grok|hbase|sam|dvasyslog|nwexp|dvamon|dvaexp|searchidx', rolename, re.IGNORECASE):
        prod_dc.extend(['prd'])
    ...
    elif re.search(r'nodeapp', rolename, re.IGNORECASE):
        prod_dc.extend(['crd'])
    elif re.search(r'sdproxy|^gdsdclient|^servicedirectory', rolename, re.IGNORECASE):
        prod_dc.extend(['prd'])
    return prod_dc
    #else:
        #return non_prod_dc

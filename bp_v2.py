...
def get_data(cluster, role, dc):
    ...
    if cluster != "NA":
        url = "https://ops0-cpt1-2-xrd.eng.sfdc.net:9876/api/v1/hosts?cluster={}&role={}&dc={}".format(cluster, role, dc)
    else:
        url = "https://ops0-cpt1-2-xrd.eng.sfdc.net:9876/api/v1/hosts?role={}&dc={}".format(role, dc)
    ...
    for host in data:
        ...
        if host['hostFailover'] == failoverstatus or failoverstatus == None:
            if host['clusterStatus'] == cl_status:
                if host['hostStatus'] in ["ACTIVE","PROVISIONING","DECOM"]:
...
if __name__ == "__main__":
    ...
    if pod == "NA" and cluster == "NA":
        ...
        url = "https://ops0-cpt1-2-xrd.eng.sfdc.net:9876/api/v1/hosts?role={}&dc={}&sp={}".format(role, dc, pod)
        data = url_response(url)
        pod_dict = {pod: []}
        for host in data:
            cluster = host['clusterName']
        ...
        url = "https://ops0-cpt1-2-xrd.eng.sfdc.net:9876/api/v1/hosts?role={}&dc={}&cluster={}".format(role, dc,cluster)
        data = url_response(url)
        single_cluster = True
